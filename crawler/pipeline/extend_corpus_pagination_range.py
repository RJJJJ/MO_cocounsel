#!/usr/bin/env python3
"""Day 20: extend pagination range and append new Macau court cases into raw corpus."""
...
REPORT_PATH = CORPUS_ROOT / "pagination_extension_report.txt"
...
def append_record_to_corpus(record: dict[str, Any], manifest_fh, new_index: int) -> None:
    ...
    metadata = {
        ...
        "extraction_source": "day20_pagination_extension",
...
def write_report(stats: ExtensionStats) -> None:
    ...
    lines = [
        ...
        f"whether pagination extension appears successful: {'yes' if appears_successful else 'no'}",
    ]
    REPORT_PATH.write_text("\n".join(lines) + "\n", encoding="utf-8")


def print_summary(stats: ExtensionStats) -> None:
    appears_successful = stats.new_corpus_records_added > 0 or stats.duplicates_skipped > 0
    ...
    print(f"whether pagination extension appears successful: {'yes' if appears_successful else 'no'}")


def run() -> int:
    playwright_error, sync_playwright = load_playwright()
    if not sync_playwright:
        stats = ExtensionStats(stop_reason="playwright is not installed")
        write_report(stats)
        ...
        with sync_playwright() as p:
            ...
            with MANIFEST_PATH.open("a", encoding="utf-8") as manifest_fh:
                ...
                for page_number in range(1, MAX_PAGE + 1):
                    ...
                    if page_number == 1:
                        target_url = submitted_result_url
                    else:
                    ...
                    if not page_cards:
                        stats.stop_reason = f"no cards found at page {page_number}"
                        break
                ...
                if not stats.stop_reason and stats.pages_attempted >= MAX_PAGE:
                    stats.stop_reason = f"reached configured max page {MAX_PAGE}"
