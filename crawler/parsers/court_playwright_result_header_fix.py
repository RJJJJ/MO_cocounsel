"""Day 16B: fix paginated result-card header token parsing.

Scope:
- reuse Day 14/15 stateful pagination flow
- parse pages 1..N (default 3)
- keep Day 16A card segmentation and Day 15 text/fulltext link preservation
- fix header parsing order: decision_date -> case_number -> case_type
...
OUTPUT_JSON_PATH = PARSED_DIR / "playwright_result_cards_paginated_header_fixed.json"
OUTPUT_REPORT_PATH = PARSED_DIR / "playwright_result_header_fix_report.txt"
...
HEADER_DATE_RE = re.compile(r"\b\d{1,2}/\d{1,2}/\d{4}\b")
HEADER_LABELS = (
    "判決/批示日期",
    "案件編號",
    "類別",
...
def strip_table_header_labels(value: str) -> str:
    cleaned = value
...
def locate_header_block(lines: list[str], raw_text: str) -> str:
    stop_keys = {
        "主題",
        "subject",
        "標的",
    ...
    for line in lines[:10]:
        low = line.lower()
        if any(k.lower() in low for k in stop_keys):
            break
    ...
    first_body_idx = len(raw_text)
    for key in stop_keys:
        idx = raw_text.lower().find(key.lower())
        if idx >= 0:
            first_body_idx = min(first_body_idx, idx)
    return normalize_space(raw_text[:first_body_idx if first_body_idx != len(raw_text) else 280])


def parse_header_tokens(lines: list[str], raw_text: str) -> tuple[str | None, str | None, str | None]:
    header_block = locate_header_block(lines, raw_text)
    if not header_block:
        return None, None, None

    cleaned = strip_table_header_labels(header_block)
    if not cleaned:
        return None, None, None

    date_match = HEADER_DATE_RE.search(cleaned)
    if not date_match:
        return None, None, None
    decision_date = normalize_space(date_match.group(0))

    after_date = normalize_space(cleaned[date_match.end() :])
    case_match = CASE_NUMBER_RE.search(after_date)
    if not case_match:
        return decision_date, None, None

    case_number = normalize_space(case_match.group(0))
    case_type = cleanup_case_type(after_date[case_match.end() :])
    return decision_date, case_number, case_type
...
def parse_card(card: dict[str, Any], court_label: str, page_number: int) -> dict[str, Any]:
    ...
    decision_date, case_number, case_type = parse_header_tokens(lines, raw_text)
...
def build_report(
    ...
    lines = [
        "# Day 16B result-card header parsing fix report",
        ...
        f"whether header fix appears successful: {metric['card_boundary_fix_success']}",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Day 16B header-token fix extractor for paginated court result cards.")
    ...
    print(f"whether header fix appears successful: {metric['card_boundary_fix_success']}")
