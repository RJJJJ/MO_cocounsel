#!/usr/bin/env python3
"""Run Day 63B bge-m3 dense retrieval baseline against Day 61 query pack."""
...
from crawler.prep.build_day63b_dense_ready_chunks import DAY63B_DENSE_READY_CHUNKS_PATH, build_day63b_dense_ready_chunks
...
DEFAULT_RESULTS_PATH = Path("data/eval/day63b_dense_retrieval_results.json")
DEFAULT_SUMMARY_PATH = Path("data/eval/day63b_dense_retrieval_summary.txt")


@dataclass(frozen=True)
class Day63BDenseResult:
    query_id: str
    query_text: str
    route_hint: str | None
...
def _normalize_hit(hit: DenseSearchHit) -> dict[str, Any]:
    return {
        ...
        "retrieval_source": "dense_day63b_bge_m3",
...
def build_summary_text(
    ...
    failed_queries: int,
    pass_rate: float,
    per_query_results: list[Day63BDenseResult],
) -> str:
    lines = [
        "Day 63B Dense Retrieval Regression Summary",
        f"run_started_at_utc: {run_started_at}",
        f"query_pack_path: {query_pack_path}",
        f"artifact_path: {artifact_path}",
        f"model_key: {DAY63B_DENSE_MODEL_CONFIG.model_key}",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Day 63B dense retrieval regression")
    parser.add_argument("--query-pack", type=Path, default=DEFAULT_QUERY_PACK_PATH)
    parser.add_argument("--top-k", type=int, default=7)
    parser.add_argument("--artifact", type=Path, default=DAY63B_DENSE_ARTIFACT_PATH)
    parser.add_argument("--source", type=Path, default=DAY63B_DENSE_READY_CHUNKS_PATH)
    parser.add_argument("--rebuild-index", action="store_true")
    parser.add_argument("--refresh-dense-ready", action="store_true")
...
def main() -> None:
    ...
    if args.refresh_dense_ready:
        build_day63b_dense_ready_chunks(output_path=args.source)

    query_specs = load_query_pack(args.query_pack)
    try:
        dense_index = build_or_load_dense_index(
        ...
        payload = {
            ...
            "model_key": DAY63B_DENSE_MODEL_CONFIG.model_key,
            "top_k": args.top_k,
            "total_queries": 0,
            "passed": 0,
            "failed": 0,
        ...
        summary_lines = [
            "Day 63B Dense Retrieval Regression Summary",
            f"run_started_at_utc: {run_started_at}",
            f"query_pack_path: {args.query_pack}",
            f"artifact_path: {args.artifact}",
            f"model_key: {DAY63B_DENSE_MODEL_CONFIG.model_key}",
            "total queries: 0",
            "passed: 0",
            "failed: 0",
            ...
            f"runtime_error: {exc}",
            "Per-query top results summary:",
            "- Not executed due to runtime error.",
            "",
            "Failure reasons:",
            "- Runtime dependency unavailable (bge-m3 backend not initialized).",
        ]
